...
Бейзлайн (файл FastText_baseline.ipynb).
...
Иерархический классификатор (файлы: Hierarhical_no_catboost.ipynb и Hierarhical_with_catboost.ipynb):
Основные модели, которые использовались при построении иерархического классификатора:
LDA (использовал gensim, пробовал BigARTM, но остановился на более простом варианте) для получения "тематических эмбеддингов" описания товара. Исходные данные для построения - список текстов-документов (Document), каждый из которых соответствует одному товару, и получен путём лемматизации и объединения в мешок слов из title (дважды), short_description и name_value_characteristics.
...
Построенная модель иерархического классификатора базируется на трёх классах (для удобства объединены в библиотеку HierarchicalLibrary):
...
load_fasttext_model - загружает с диска предварительно обученную модель fasttext
...
fasttext_encoder - функция, преобразующая лемматизированный документ в fasttext-word2vec вектор документа
...
Метрика иерархической F-меры получилась ниже бейзлайна (максимум после всех экспериментов и доработок, указанных ниже, hF1=0,89), хотя модель значительно сложнее как вычислительно, так и архитектурно.
...
Для сабмита подобрал гиперпараметры и обучил бейзлайн (файл SUBMIT.ipynb) на полном датасете. Итог - 1 место в лидерборде. Простейший baseline, сделанный за пару часов + пару ночей grid search позволили выйти в лидеры, а две недели экспериментов со иерархическим классификатором сложной структуры не увенчались никаким успехом, кроме полученного опыта о том, что простым задачам - простые решения.
